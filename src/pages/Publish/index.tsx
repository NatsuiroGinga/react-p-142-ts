...
const Publish = () => {
    ...
    const onUploadChange = ({fileList}) => {
        // 同时把图片列表存入仓库一份
        // 这里关键位置:需要做数据格式化
        const formatList = fileList.map((file: { response: { data: { url: any } } }) => {
            // 上传完毕 做数据处理
            if (file.response) {
                return {
    ...
    const radioChange = (e: { target: { value: number } }) => {
    ...
    const onFinish = async (values: { channel_id: any; content: any; title: any; type: any }) => {
